...
from .stats import R, R_mass, Rf, PofR
...
def plot_All(f, p, *, a=0, b=1):
    ...
    ax.plot(ps, f(ps));
    plot_R(f, r, a, b, ax=ax, alpha=0.5, color='lightblue')
    ax.axhline(p, c='darkblue', linewidth=1)
    ax.text(0.6, 0.8, f"r={p:.3f}  $P\\left(x\\in R(r)\\right) = {PofR(f, r):.3f}$", transform=plt.gca().transAxes, fontsize=16);
